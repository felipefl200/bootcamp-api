...
# Regras de Indexação do Codebase

## Objetivo

Garantir que assistentes de IA tenham uma visão atualizada da estrutura do projeto antes de fazer alterações.

## Regras

1. **Antes de modificar qualquer arquivo**, verifique a estrutura atual do projeto executando:

   ```bash
   bash .agent/skills/indexing-awareness/scripts/index-structure.sh
   ```

2. **Antes de alterar rotas ou dependências**, trace as dependências com:

   ```bash
   bash .agent/skills/indexing-awareness/scripts/trace-dependencies.sh
   ```

3. **Nunca assuma a existência de um arquivo** — sempre confirme antes.

4. **Diretórios ignorados** (nunca indexar/modificar):
   - `node_modules/`
   - `dist/`
   - `src/generated/`
   - `.git/`

5. **Ao criar novos arquivos**, atualize o `AGENTS.md` se a mudança afetar a estrutura documentada.

## Quando Re-indexar

- Após criar novos arquivos ou diretórios
- Após instalar/remover dependências
- Após alterações no schema do Prisma
- Quando houver dúvida sobre a estrutura atual
